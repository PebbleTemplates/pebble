...
| `allowUnsafeMethods` | If set to false, Pebble will throw an exception if you try to access unsafe methods. Unsafe methods are defined [here](https://github.com/PebbleTemplates/pebble/tree/master/pebble/src/main/resources/unsafeMethods.properties). All methods of class `java.lang.Class` and `java.lang.Runtime` are also considered unsafe.| `true` in 2.x, 'false' in v3.x |
